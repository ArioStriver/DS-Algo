...
class Solution {
    public int minDeletions(String s) {
        ...
        // first storing the frequency of each character
        for(char c : s.toCharArray()) {
            freq[c-'a']++;
        }

        Set<Integer> unique_freq = new HashSet<>(); // store the unique frequency
        int minDeletionNeeded = 0;

        for(int i = 0; i < freq.length; i++) {
            // current frequency is already present and also frequency of 0 is ignored i.e. > 0
            while(freq[i] > 0 && unique_freq.contains(freq[i])) {
                freq[i]--;           // delete the current charcater
                minDeletionNeeded++;  
            }
            unique_freq.add(freq[i]);
        }

        return minDeletionNeeded;
    }
}